...
public class WMITask {
	...
	public boolean connect(String servername, String namespace, String username, String password) {
		logger.fine("connection with server "+servername);
		this.servername = servername;
		this.namespace = namespace;
		this.username = username;
		this.password = password;
		
		String vbs = "Set objLocator = CreateObject(\"WbemScripting.SWbemLocator\")"+CRLF;
		vbs += "Set objService = objLocator.ConnectServer(\""+servername+"\", \""+namespace+"\",\""+username+"\",\""+password+"\")"+CRLF;
		vbs += "objService.Security_.ImpersonationLevel = 3"+CRLF;
		vbs += "Set colSWbemObjectSet = objService.ExecQuery(\"Select * from Win32_Process Where Name = 'explorer.exe'\")"+CRLF;
		vbs += "wscript.echo colSWbemObjectSet.Count"+CRLF;
		
		String tmpDirName = System.getProperty("mxssh.path");
		String tmpFileName = tmpDirName + File.separator + "jwmi.vbs";
		try {
			writeStrToFile(tmpFileName, vbs);
	...
	private String getVBScript(String wmiQueryStr, String wmiCommaSeparatedFieldName){
		String vbs = "Set objLocator = CreateObject(\"WbemScripting.SWbemLocator\")"+CRLF;
		vbs += "Set objService = objLocator.ConnectServer(\""+this.servername+"\", \""+this.namespace+"\",\""+this.username+"\",\""+this.password+"\")"+CRLF;
		vbs += "objService.Security_.ImpersonationLevel = 3"+CRLF;
		vbs += "Set colSWbemObjectSet = objService.ExecQuery(\""+wmiQueryStr+"\")"+CRLF;
		vbs += "strData = \"\""+CRLF;		
		vbs += "For Each objSWbemObject in colSWbemObjectSet"+CRLF;
		String[] wmiFieldNameArray = wmiCommaSeparatedFieldName.split(",");
		for(int i = 0; i < wmiFieldNameArray.length; i++)
		{
			vbs += "	strData = strData & objSWbemObject."+wmiFieldNameArray[i]+" & VBCrLf"+CRLF;
		}		
	...
	public String getWMIValue(String wmiQueryStr, String wmiCommaSeparatedFieldName) throws Exception	{
		logger.fine("getWMIValue()");
		String vbScript = getVBScript(wmiQueryStr, wmiCommaSeparatedFieldName);
		String tmpDirName = System.getProperty("mxssh.path");
		logger.fine("mxssh.path="+tmpDirName);
		String tmpFileName = tmpDirName + File.separator + "jwmi.vbs";
		logger.fine("tmpFileName="+tmpFileName);
	...
	private String execute(String[] cmdArray) throws Exception	{
		logger.fine("execute");
	...
	public void executeDemoQueries()	{
		try
		{
	...
	public static final String CLASS_Win32_OSRecoveryConfiguracion = "Win32_OSRecoveryConfiguración";
